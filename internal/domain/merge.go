...
func Equivalent(a, b Operation) bool {
	if a.ID != b.ID || a.Record != b.Record || a.Field != b.Field || a.Value != b.Value {
		return false
	}
...
func Frontier(ops []Operation, record, field string) []Operation {
	...
	for _, o := range ops {
		if o.Record == record && o.Field == field {
	...
	result := []Operation{}
	for _, o := range ops {
		if o.Record == record && o.Field == field && !removed[o.ID] {
...
func ValidateBatch(existing map[string]Operation, batch []Operation) error {
	...
	for _, o := range batch {
		...
		for _, p := range o.Parents {
			parent, ok := known[p]
			if !ok || parent.Record != o.Record || parent.Field != o.Field {
				return ErrInvalid
			}
		}
		known[o.ID] = o
