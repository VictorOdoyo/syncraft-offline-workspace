...
func (h *Hub) Notify(w string) {
	h.mu.Lock()
	defer h.mu.Unlock()
	for ch := range h.listeners[w] {
		select {
		case ch <- struct{}{}:
		default:
		}
...
func (h *Hub) subscribe(w string) (chan struct{}, func()) {
	h.mu.Lock()
	defer h.mu.Unlock()
	if h.listeners[w] == nil {
		h.listeners[w] = map[chan struct{}]bool{}
	}
	ch := make(chan struct{}, 1)
	h.listeners[w][ch] = true
	return ch, func() {
		h.mu.Lock()
		defer h.mu.Unlock()
		delete(h.listeners[w], ch)
		if len(h.listeners[w]) == 0 {
			delete(h.listeners, w)
		}
	}
}
func (s *Server) events(w http.ResponseWriter, r *http.Request) {
	...
	valid := func() bool {
		d, err := s.Store.Device(r.Context(), claims.Workspace, hello.Device)
		return err == nil && !d.Revoked && d.Actor == claims.Subject && claims.ExpiresAt.After(time.Now())
	}
	if !valid() {
		_ = conn.Close(websocket.StatusPolicyViolation, "device denied")
		return
	}
	ch, remove := s.Hub.subscribe(claims.Workspace)
	defer remove()
	...
	send := func() error {
		...
		defer cancel()
		return conn.Write(ctx, websocket.MessageText, []byte(`{"type":"sync-needed"}`))
	}
	if send() != nil {
		return
	}
