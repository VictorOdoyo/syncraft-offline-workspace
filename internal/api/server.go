...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /health/ready", func(w http.ResponseWriter, r *http.Request) {
		if err := s.Store.Ping(r.Context()); err != nil {
			write(w, 503, map[string]string{"error": "storage unavailable"})
			return
		}
...
func problem(w http.ResponseWriter, err error) {
	status, msg := 500, "internal service error"
	switch {
...
func (s *Server) identity(w http.ResponseWriter, r *http.Request, edit, device bool) (auth.Claims, bool) {
	token := strings.TrimPrefix(r.Header.Get("Authorization"), "Bearer ")
	c, err := s.Auth.Verify(token)
	if err != nil {
		write(w, 401, map[string]string{"error": "authentication required"})
		return c, false
	}
	if edit && c.Role != "editor" && c.Role != "admin" {
		problem(w, domain.ErrForbidden)
		return c, false
	}
	if device {
		d, err := s.Store.Device(r.Context(), c.Workspace, r.Header.Get("X-Device-ID"))
		if err != nil || d.Revoked || d.Actor != c.Subject {
			problem(w, domain.ErrForbidden)
			return c, false
		}
	}
...
func (s *Server) login(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Username) > 100 || len(req.Password) > 256 {
		problem(w, domain.ErrInvalid)
		return
	}
...
func (s *Server) register(w http.ResponseWriter, r *http.Request) {
	c, ok := s.identity(w, r, false, false)
	if !ok {
		return
	}
	var req struct {
	...
	if !domain.ValidID(req.ID) || strings.TrimSpace(req.Name) == "" || len(req.Name) > 100 {
		problem(w, domain.ErrInvalid)
		return
	}
	d := domain.Device{ID: req.ID, Name: req.Name, Actor: c.Subject}
	if err := s.Store.Register(r.Context(), c.Workspace, d); err != nil {
		problem(w, err)
		return
	}
	write(w, 201, d)
...
func (s *Server) devices(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.Store.Devices(r.Context(), c.Workspace)
	if err != nil {
		problem(w, err)
		return
	}
...
func (s *Server) revoke(w http.ResponseWriter, r *http.Request) {
	...
	d, err := s.Store.Device(r.Context(), c.Workspace, id)
	if err != nil {
		problem(w, err)
		return
	}
	if c.Role != "admin" && d.Actor != c.Subject {
		problem(w, domain.ErrForbidden)
		return
	}
	if err = s.Store.Revoke(r.Context(), c.Workspace, id, c.Subject); err != nil {
		problem(w, err)
		return
	}
...
func (s *Server) push(w http.ResponseWriter, r *http.Request) {
	...
	cursor, err := s.Store.Push(r.Context(), c.Workspace, r.Header.Get("X-Device-ID"), c.Subject, req.Operations)
	if err != nil {
		problem(w, err)
		return
	}
...
func (s *Server) pull(w http.ResponseWriter, r *http.Request) {
	...
	page, err := s.Store.Pull(r.Context(), c.Workspace, after, 100)
	if err != nil {
		problem(w, err)
		return
	}
...
func (s *Server) audit(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.Store.Audit(r.Context(), c.Workspace, after, 100)
	if err != nil {
		problem(w, err)
		return
	}
