...
class ApiClient {
  ...
  String? token;
  String? device;
  ApiClient(String endpoint, {http.Client? client})
    ...
    if (base.scheme != 'https' &&
      ...
      throw const FormatException('Remote servers require HTTPS');
    }
  }
      ...
      await transport
          .post(
            ...
            headers: {
              ...
              'Content-Type': media,
              'X-Record-ID': record,
              'X-Filename': Uri.encodeQueryComponent(name),
            },
            body: bytes,
          )
  ...
  Future<Uint8List> download(String id) async {
    final response = await transport
        .get(uri('/api/v1/attachments/$id'), headers: headers)
        .timeout(const Duration(seconds: 30));
