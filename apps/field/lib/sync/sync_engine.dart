...
class SyncEngine extends ChangeNotifier {
  ...
  Future<void> login(String username, String password) async {
    ...
    if (existing != null && existing != binding) {
      throw const FormatException(
        'This device is bound to another account. Export recovery data before using a separate browser profile.',
      );
    }
    ...
    api.device = await store.deviceId();
    try {
      await api.post('/api/v1/devices', {
        'id': api.device,
        'name': 'Syncraft field device',
      });
    } catch (_) {
      api.token = null;
      rethrow;
    }
    await store.setMetadata('account', binding);
    connected = true;
    error = null;
    _notify();
    await synchronize();
  ...
  Future<void> synchronize() async {
    if (busy || paused || !connected || _disposed) {
      return;
    }
    busy = true;
    error = null;
    ...
    try {
      ...
      for (var i = 0; i < 100; i++) {
        ...
        try {
          ...
          await store.failed(pending.map((o) => o.id).toList(), e.toString());
          rethrow;
        }
      ...
      for (var i = 0; i < 100; i++) {
        final page = Map<String, dynamic>.from(
          await api.get('/api/v1/sync/pull?after=${await store.cursor()}')
              as Map,
        );
  ...
  void _schedule() {
    ...
    _timer = Timer(
      Duration(
        seconds: failures == 0
            ? 15
            : (5 * (1 << failures.clamp(0, 6))).clamp(5, 300),
      ),
      () async {
        await synchronize();
        _schedule();
