...
class _AttachmentPanelState extends State<AttachmentPanel> {
  late final store = AttachmentStore(widget.controller.store);
  ...
  Future<void> refresh() async {
    final result = await store.list(widget.record);
    if (mounted) {
    ...
    children: [
      Wrap(
        ...
        children: [
          ...
          IconButton(
            tooltip: 'Download remote attachments',
            onPressed: busy || !widget.controller.sync.connected
                ? null
                : () => run(
                    () =>
                        store.fetch(widget.controller.sync.api, widget.record),
                  ),
      ...
      for (final row in rows)
        ListTile(
          ...
          subtitle: Text(
            row['uploaded'] == 1 ? 'Available offline' : 'Pending upload',
          ),
          trailing: IconButton(
