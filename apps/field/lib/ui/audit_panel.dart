...
class AuditPanel extends StatefulWidget {
  final ApiClient api;
  const AuditPanel({super.key, required this.api});
  @override
...
class _AuditPanelState extends State<AuditPanel> {
  ...
  Future<void> load() async {
    ...
    setState(() => busy = true);
    try {
      final after = rows.isEmpty ? 0 : rows.last['sequence'];
      final result = await widget.api.get('/api/v1/audit?after=$after') as List;
      if (mounted) {
        setState(() {
          rows.addAll(result);
  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      ...
      if (more && !busy)
        TextButton(
          onPressed: load,
          child: Text(error == null ? 'Load more' : 'Retry'),
        ),
    ],
  );
}
