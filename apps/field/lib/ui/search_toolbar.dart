...
class SearchToolbar extends StatefulWidget {
  ...
  const SearchToolbar({super.key, required this.controller});
  @override
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                ...
                onPressed: () => c.filter(text: ''),
                icon: const Icon(Icons.clear),
              ),
            ),
            onChanged: (value) => c.filter(text: value),
          ),
            ...
            children: [
              SizedBox(
                ...
                child: DropdownButtonFormField<String>(
                  ...
                  items: ['all', 'draft', 'in_progress', 'complete']
                      .map(
                        (s) =>
                            DropdownMenuItem(value: s, child: Text(label(s))),
                      )
                      .toList(),
                  onChanged: (v) => c.filter(state: v),
                ),
              ),
              SizedBox(
                width: 160,
                child: DropdownButtonFormField<String>(
                  ...
                  items: ['all', 'normal', 'high', 'critical']
                      .map((s) => DropdownMenuItem(value: s, child: Text(s)))
                      .toList(),
                  onChanged: (v) => c.filter(urgency: v),
                ),
              ),
              FilterChip(
                label: const Text('Conflicts'),
                selected: c.conflictsOnly,
                onSelected: (v) => c.filter(conflicts: v),
              ),
              FilterChip(
                label: const Text('Archived'),
                selected: c.showArchived,
                onSelected: (v) => c.filter(archived: v),
              ),
              IconButton(
