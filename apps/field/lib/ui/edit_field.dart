...
class _FieldDialogState extends State<_FieldDialog> {
  late final TextEditingController text = TextEditingController(
    text: widget.observed.firstOrNull?.value ?? '',
  );
  bool saving = false;
  String? error;
  @override
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    content: SizedBox(
      ...
      child: TextField(
        controller: text,
        autofocus: true,
        maxLines: widget.field == 'notes' ? 8 : 1,
        maxLength: fieldLimits[widget.field],
        decoration: InputDecoration(
          labelText: widget.field == 'notes' ? 'Field notes' : widget.field,
          errorText: error,
        ),
      ),
    ),
    actions: [
      TextButton(
        onPressed: saving
            ? null
            : () async {
                final original = widget.observed.firstOrNull?.value ?? '';
                if (text.text == original ||
                    await confirm(
                      context,
                      'Discard changes?',
                      'The unsaved edit will be discarded.',
                      'Discard',
                    )) {
                  if (context.mounted) {
            ...
            : () async {
                ...
                try {
                  await widget.controller.edit(
                    ...
                    widget.field,
                    text.text,
                    widget.observed.map((o) => o.id).toList(),
                  );
                  if (context.mounted) {
                    Navigator.pop(context);
                  }
