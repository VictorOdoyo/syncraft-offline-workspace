...
class LocalStore {
  ...
  static Future<LocalStore> open() async =>
      LocalStore(await openLocalDatabase('syncraft-v1.db', databaseOptions));
  ...
  Future<String> deviceId() async {
    return db.transaction((tx) async {
      ...
      await tx.insert('metadata', {'key': 'device', 'value': id});
      return id;
    });
  }

  Future<List<Operation>> operations([DatabaseExecutor? executor]) async =>
      (await (executor ?? db).query('operations', orderBy: 'position'))
          .map(
            (r) => Operation.fromJson(
  ...
  Future<void> save(List<Operation> batch) async {
    await db.transaction((tx) async {
      ...
      await replica.addAll(await operations(tx));
      await replica.addAll(batch);
  ...
  Future<List<Operation>> pending() async =>
      (await db.rawQuery(
            'SELECT o.content FROM operations o JOIN outbox q ON q.id=o.id ORDER BY o.position LIMIT 100',
          ))
          .map(
            (r) => Operation.fromJson(
              jsonDecode(r['content'] as String) as Map<String, dynamic>,
  ...
  Future<int> pendingCount() async =>
      (await db.rawQuery('SELECT count(*) AS n FROM outbox')).first['n'] as int;
  Future<void> acknowledge(List<String> ids) async {
    await db.transaction((tx) async {
      for (final id in ids) {
        await tx.delete('outbox', where: 'id=?', whereArgs: [id]);
      }
    });
  }

  Future<void> failed(List<String> ids, String error) async {
    await db.transaction((tx) async {
      for (final id in ids) {
        await tx.rawUpdate(
  ...
  Future<void> applyPage(Map<String, dynamic> page) async {
    ...
    await db.transaction((tx) async {
      final previousRows = await tx.query(
        'metadata',
        where: 'key=?',
        whereArgs: ['cursor'],
      );
      final previous = int.parse(
        previousRows.firstOrNull?['value'] as String? ?? '0',
      );
      if (next < previous || (entries.isEmpty && next != previous)) {
        throw const FormatException('Invalid sync cursor');
      }
      var expected = previous;
      for (final entry in entries) {
        if (entry['sequence'] != ++expected) {
          throw const FormatException('Noncontiguous sync page');
        }
      }
      if (expected != next) {
        throw const FormatException('Page cursor mismatch');
      }
      ...
      for (var i = 0; i < incoming.length; i++) {
        ...
        await tx.update(
          'operations',
          {'actor': entry['actor'], 'created': entry['created']},
          where: 'id=?',
          whereArgs: [op.id],
        );
      }
